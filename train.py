...
for epoch in range(conf.nepoch):
    ...
    for b in range(len(dataloader_train)):
        ...
        if (b+1) % log_iter == 0 and (b+1) >= log_iter:
            ...
            start = time.time()
    # max_batch = {k: v for k, v in sorted(max_batch.items(), key=lambda item: item[1],reverse=True)}
    # print(list(max_batch.keys())[0],max_batch[list(max_batch.keys())[0]])
    # exit()
    if not conf.no_logging:
    ...
    # print("*" * 40, flush=True)
    # print("save the checkpoint after {} epochs".format(epoch), flush=True)
